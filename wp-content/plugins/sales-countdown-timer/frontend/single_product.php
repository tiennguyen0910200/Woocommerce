...
//		    calculate sold quantity during campaign
			$data                       = get_post_meta( $variation_id, '_woo_ctr_product_sold_quantity', true ) ? ( get_post_meta( $variation_id, '_woo_ctr_product_sold_quantity', true ) ) : array();
			...
			if ( is_array( $data ) && count( $data ) && is_array( $order_status ) ) {
				foreach ( $data as $key => $value ) {
					$order = get_post( $value['id'] );
					if ( $order && ( ! count( $order_status ) || in_array( $order->post_status, $order_status ) ) ) {
		...
		if ( $progress_bar_real_quantity >= 0 && $progress_bar && $progress_bar_goal ) {
            ...
            <div class="woo-sctr-progress-bar-wrap-container">
				...
				if ( '' !== $progress_bar_border_radius ) {
                    ...
                    <div class="woo-sctr-progress-bar-fill" style="width: <?php echo $progress_bar_fill . '%;';
					if ( $progress_bar_color )
						echo 'background:' . $progress_bar_color . ';' ?>"></div>
                </div>
				<?php
				?>
